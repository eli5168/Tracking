...
max_train_samples = params.nSamples; %max stored smaples.
...
disp('---------')
%disp(features{1}.fparams.net.info.receptiveFieldStride)
...
disp('feature_sz')
disp(feature_sz) % feature_sz : each output layer size e.g. [52,52],[13,13]
...
%disp('length===feature_sz_cell')
%disp(feature_sz_cell{3})
...
for frame = 1:num_frames
    ...
    if frame > 1
        ...
        while iter <= params.refinement_iterations && any(old_pos ~= pos)
            ...
            %disp(size(xtf_proj{1,3}))  27    27    16     7
           ...
           % disp(scores_fs_feat{1}(:,:,1,7))
            %scores_fs_feat{1}=2.4*scores_fs_feat{1}
            %scores_fs_feat{2}=1.6*scores_fs_feat{2}
            disp('---------after coff--------------')
           % disp(scores_fs_feat{1}(:,:,1,7))
            %scores_fs_feat{1,1,1}= scores_fs_feat{1,1,1};%53 53 1 7
            %scores_fs_feat{1,1,2} =scores_fs_feat{1,1,2}*2; %53 53 1 7
            %scores_fs_feat{1,3} ...
            ...
            %disp(size(scores_fs)) 53 53 7
            
            % Optimize the continuous score function with Newton's method.
            [trans_row, trans_col, max_scale_response,scale_ind] = optimize_scores(scores_fs, params.newton_iterations);
...
%         set(gcf,'units','normalized','outerposition',[0 0 1 1]);
        ...
        for disp_layer = 1:num_feature_blocks;
            ...
            if frame > 1
                ...
                colorbar;
                axis image;
                outputImg1='./save_debug/'
                outputImgName1 = [outputImg1,num2str(frame),'.jpg'];
                frame_2 = getframe;            
                saveas(gcf, outputImgName1, 'jpg');
            end
        end
        ...
        if train_tracker
            ...
            outputImg2='./save_train/'
            outputImgName2 = [outputImg2,num2str(frame),'.jpg'];
            frame_3 = getframe;            
            saveas(gcf, outputImgName2, 'jpg'); 
        end
    end
...
%                 set(fig_handle, 'Position', [100, 100, 100+size(im,2), 100+size(im,1)]);
            ...
            rectangle('Position',rect_position_vis, 'EdgeColor','g', 'LineWidth',2);
            text(10, 10, int2str(frame), 'color', [0 1 1]);
            text(10, 20, num2str( max_scale_response), 'color', [0 1 1]);
           

            outputImg='./save_visual/'
            outputImgName = [outputImg,num2str(frame),'.jpg'];
            frame_ = getframe;            
