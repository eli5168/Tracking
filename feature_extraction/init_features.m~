...
for n = 1:length(features)
    ...
    if ~isfield(features{n}.fparams,'use_gpu')
        features{n}.fparams.use_gpu = true;
    end
    if ~isfield(features{n}.fparams, 'return_gpu')
           %features{k}.fparams.return_gpu = true;
           features{n}.fparams.return_gpu = false;
    end
    if ~isfield(features{n}.fparams, 'gpu_id')
      %     %features{k}.fparams.return_gpu = true;
          features{n}.fparams.gpu_id = [1];
    end
...
for k = 1:length(features)
    ...
    elseif isequal(features{k}.getFeature, @get_cnn_layers) || isequal(features{k}.getFeature, @get_OFcnn_layers)
        ...
        %disp(features{k}.fparams.output_layer)
       ...
       if ~isfield(features{k}.fparams, 'gpu_id')
      %    %features{k}.fparams.return_gpu = true;
        
         features{k}.fparams.gpu_id = [1];
        end
    ...
    %disp(feature_info.min_cell_size(k))
end
...
if cnn_feature_ind > 0
    ...
    net = load_cnn(features{cnn_feature_ind}.fparams, img_sample_sz);
    net_info = net.info;
    ...
    %disp(features{cnn_feature_ind}.fparams)
    %
...
for k = 1:length(features)
    if features{k}.is_cnn
        ...
        %disp(features{k}.fparams.net.info.receptiveFieldStride)
        feature_info.data_sz_block{k} = floor(bsxfun(@rdivide, cnn_output_sz, features{k}.fparams.downsample_factor'));
        %disp(cnn_output_sz)
        %disp(features{k}.fparams)
        %disp(feature_info.data_sz_block{k})
    else
